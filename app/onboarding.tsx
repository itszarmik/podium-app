...
{ key: 'realtime', icon: Zap, iconColor: colors.indigoGlow, iconBg: `${colors.indigo}25`, title: 'Watch rankings\nchange live', sub: 'See rank changes as they happen. Animated updates and instant notifications.', accent: colors.indigoGlow },
{ key: 'teams', icon: Users, iconColor: colors.teal, iconBg: `${colors.teal}20`, title: 'Build boards\nfor your team', sub: 'Sales, fitness, gaming, music - create a custom leaderboard in seconds.', accent: colors.teal },
{ key: 'share', icon: Share2, iconColor: colors.green, iconBg: `${colors.green}20`, title: 'Share and\ngo viral', sub: 'One tap to share your board link. Public boards are discoverable by anyone.', accent: colors.green },
]
...
export default function OnboardingScreen() {
  ...
  return (
    <SafeAreaView style={{flex:1,backgroundColor:colors.black}} edges={['top','bottom']}>
      {!isLast && <TouchableOpacity style={{position:'absolute',top:52,right:20,zIndex:10,paddingHorizontal:14,paddingVertical:7,backgroundColor:colors.surface,borderRadius:999,borderWidth:0.5,borderColor:colors.border}} onPress={finish} activeOpacity={0.7}><Text style={{color:colors.sub,fontSize:13,fontWeight:'600'}}>Skip</Text></TouchableOpacity>}
      <Animated.FlatList ref={flatRef} data={SLIDES} keyExtractor={(s)=>s.key} horizontal pagingEnabled showsHorizontalScrollIndicator={false} scrollEventThrottle={16} onScroll={Animated.event([{nativeEvent:{contentOffset:{x:scrollX}}}],{useNativeDriver:false})} onMomentumScrollEnd={(e)=>setIndex(Math.round(e.nativeEvent.contentOffset.x/width))} renderItem={({item})=><view style={{width,paddingHorizontal:24,paddingTop:height*0.08,alignItems:'center'}}><view style={{width:120,height:120,borderRadius:32,alignItems:'center',justifyContent:'center',backgroundColor:item.iconBg,borderWidth:1,borderColor:`${item.iconColor}30`,marginBottom:32}}><item.icon size={52} color={item.iconColor} strokeWidth={1.5}/></view><text style={{fontSize:32,fontWeight:'800',color:colors.text,textAlign:'center',lineHeight:40,letterSpacing:-0.8,marginBottom:16}}>{item.title}</text><text style={{fontSize:16,color:colors.sub,textAlign:'center',lineHeight:24,paddingHorizontal:16}}>{item.sub}</text></view>}/>
      <View style={{paddingHorizontal:24,paddingBottom:Platform.OS==='ios'?16:24,alignItems:'center',gap:16}}>
        <View style={{flexDirection:"row",gap:6,alignItems:'center'}}>
          {SLIDES.map((_,i)=>{
            ...
            return <Animated.View key={i} style={[{height:8,borderRadius:4},{width:scrollX.interpolate({inputRange:ir,outputRange:[8,24,8],extrapolate:'clamp'}),opacity:scrollX.interpolate({inputRange:ir,outputRange:[0.35,1,0.35],extrapolate:'clamp'}),backgroundColor:SLIDES[index].accent}]} />
          })}
        </View>
